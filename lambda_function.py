...
s3 = boto3.resource('s3', region_name='us-west-2', aws_access_key_id='',aws_secret_access_key='')
session = boto3.session.Session()
ec2 = session.client('ec2', region_name='us-east-1', aws_access_key_id='',aws_secret_access_key='')
...
def create_spot_instance(workload, region_ami, az_map_dict):
    ...
    arm64_family = ['a1', 't4g', 'c7g', 'c7gn', 'c6g', 'c6gd', 'c6gn', 'im4gn', 'is4gen', 'm7g', 'm6g', 'm6gd', 'r6g', 'r6gd', 'x2gd']
...
def lambda_handler(event, context):
    ...
    try:
        ...
        response = ec2.describe_spot_instance_requests(Filters=[{'Name': 'state', 'Values': ['open']}])
        spot_instance_requests = []
        for sir in response["SpotInstanceRequests"]:
            spot_instance_requests.append(sir["SpotInstanceRequestId"])
        ec2.cancel_spot_instance_requests(SpotInstanceRequestIds=spot_instance_requests)
